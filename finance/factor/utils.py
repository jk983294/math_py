...
def quantize_factor(factor_data,
    ...
    def quantile_calc(x, _quantiles, _bins, _zero_aware, _no_raise):
        try:
            if _quantiles is not None and _bins is None and not _zero_aware:
                quantiles_ = pd.qcut(x, _quantiles, labels=False) + 1
                return quantiles_
    ...
    if len(factor_quantile.index.levels) == 3:
        factor_quantile = factor_quantile.droplevel(0)
...
def get_clean_factor(factor,
    ...
    if groupby is not None:
        if isinstance(groupby, dict):
            diff = set(factor_copy.index.get_level_values('asset')) - set(groupby.keys())
            if len(diff) > 0:
                raise KeyError(
